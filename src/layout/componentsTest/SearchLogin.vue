<template>
  ...
  <div class="login-tips" style="z-index: 1">
    <a-input style="width: 180px; font-size: 18px;height: 32px;" ref="inputs" size="default"
      v-if="!currentPath.startsWith('/login') && currentPath!='/indexSearching'" v-model:value="searchValue"
      placeholder="请输入">
      <template #suffix>
        <div @click="onSearch">
    ...
    <span v-if="!isLogged && !currentPath.startsWith('/login')" data-sensors-click class="center login-btn"
      style="margin-left: 20px" @click="login">
      登录
    </span>
    <!-- 登陆中 -->
    <span v-if="isLogin === 1 && route.path.includes('login')" class="center shift-btn" @click="shiftIns">
    ...
    <span v-if="false" class="center login-btn" style="margin-left: 20px" @click="back">
      返回
    </span>
    <!-- 已经登录 -->
    <div v-if="isLogged" class="menu">
      ...
      <span style="padding-left:30px;font-family:'Microsoft Yahei';color:#000;" @click="handleAction(1)">用户中心</span>
    </div>
  </div>
</template>
